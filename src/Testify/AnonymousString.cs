...
public static class AnonymousString
{
    ...
    public static string AnyFirstName(this IAnonymousData anon, bool? male = default)
    {
        Guard.Against.Null(anon);

        var names = (male ?? anon.AnyBool()) ? MaleFirstNames : FemaleFirstNames;
    ...
    /// <exception cref="System.ArgumentNullException"><paramref name="anon"/> is <see langword="null"/>.</exception>
    public static string AnyFullName(this IAnonymousData anon, bool? male = default)
    {
        Guard.Against.Null(anon);

        var isMale = male ?? anon.AnyBool();
        var prefixes = isMale ? MalePrefixes : FemalPrefixes;
        switch (anon.AnyInt32(0, 10))
        {
            case 0:
                return $"{anon.AnyItem(prefixes)} {anon.AnySimpleFullName(isMale)}";

            case 1:
                return $"{anon.AnySimpleFullName(isMale)} {anon.AnyItem(Suffixes)}";

            case 2:
                return $"{anon.AnySimpleFullName(isMale)} {anon.AnyItem(GenerationalSuffixes)}";

            default:
                return anon.AnySimpleFullName(isMale);
        }
    }
    ...
    /// <param name="charGenerator">The character genreator. Defaults to creating anonymous alpha numericd characters.</param>
    ...
    public static string AnyString(
        ...
        Func<IAnonymousData, char>? charGenerator = default)
    {
        Guard.Against.Null(anon);
        Guard.Against.OutOfRange(maximumLength, minimumLength, maximumLength);

        var generator = charGenerator ?? (a => a.AnyAlphaChar());
        var length = anon.AnyInt32(minimumLength, maximumLength);
        var builder = new StringBuilder(length);
        while (builder.Length < length)
        {
            builder.Append(generator(anon));
        }
